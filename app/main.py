...
app = FastAPI(title="AiWebCity", version="0.2.0")
...
def parse_generated_image(body: dict[str, Any]) -> bytes | None:
    # Accept several common image-output shapes so the adapter survives provider-side changes.
...
"""}, {"type": "image_url", "image_url": {"url": data_url(modern_bytes, modern_type)}}]
    ...
    for item in evidence:
        meta = item["meta"]
        evidence_text.append(f"Источник: {meta.get('source', 'не указан')}; год: {meta.get('year', 'не указан')}; описание: {meta.get('description', '')}")
        content.append({"type": "text", "text": evidence_text[-1]})
...
async def reconstruct(
    ...
    return {
        ...
        "message": "Готово. Архивные источники показаны вместе с реконструкцией.",
    }
